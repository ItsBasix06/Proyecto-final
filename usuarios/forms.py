...
class ClienteEditarForm(forms.ModelForm):
    """Formulario definitivo sin el campo de suscripción."""
    class Meta:
        model = Cliente
        # Dejamos solo los campos que sí quieres que edite el arquitecto
        fields = ['nom_empresa', 'telefono', 'correo_electronico', 'cuenta_bancaria']
        widgets = {
            'nom_empresa': forms.TextInput(attrs={'class': 'form-control rounded-3'}),
            'telefono': forms.TextInput(attrs={'class': 'form-control rounded-3'}),
            'correo_electronico': forms.EmailInput(attrs={'class': 'form-control rounded-3'}),
            'cuenta_bancaria': forms.TextInput(attrs={'class': 'form-control rounded-3', 'placeholder': 'ES00...'}),
        }